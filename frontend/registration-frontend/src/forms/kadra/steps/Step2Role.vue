<template>
  <div class="bg-white rounded-2xl shadow p-6 space-y-6">
    ...
    <div v-if="!isAdult" class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800">
      Osoby niepełnoletnie mogą pełnić następujące role: <strong>Kandydat na animatora</strong>, <strong>Sternik z opiekunem</strong>.
    </div>
...
const availableRoles = computed(() => {
  return allRoles.filter(role => props.isAdult || !role.adultOnly)
})
