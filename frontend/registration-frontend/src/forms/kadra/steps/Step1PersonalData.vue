  ...
  <div class="bg-white rounded-2xl shadow p-6 space-y-6">
    ...
    <div>
      ...
      <div v-if="loadingTurnusy" class="flex items-center gap-2 text-gray-500 text-sm">
        <span
            class="animate-spin inline-block w-4 h-4 border-2 border-gray-400 border-t-transparent rounded-full"></span>
        Ładowanie…
      </div>
      ...
      <div v-if="selectedTurnus" class="mt-3 border border-gray-200 rounded-lg p-3 bg-gray-50 text-sm space-y-1">
        ...
        <div><span class="font-medium">Termin:</span> {{ selectedTurnus.startDate }} – {{ selectedTurnus.endDate }}
        </div>
    ...
    <div v-if="!isAdult && pesel.length === 11" class="border border-yellow-200 rounded-lg p-4 bg-yellow-50 space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">E-mail *</label>
          <input
              v-model="local.guardianEmail"
              type="email"
              :class="fieldClass('guardianEmail')"
          />
          <p v-if="errors.guardianEmail" class="text-red-500 text-xs mt-1">
      ...
      <div>
        ...
        />
        <p class="text-xs text-gray-400 mt-1">Wypełnione automatycznie na podstawie danych opiekuna — można
          edytować.</p>
    ...
    <div v-if="isAdult && pesel.length === 11" class="border border-blue-200 rounded-lg p-4 bg-blue-50 space-y-4">
      ...
      <div v-if="local.iceRelation === 'inna'">
        ...
        <input v-model="local.iceRelationOther" type="text" placeholder="np. brat, znajomy"
               :class="fieldClass('iceRelationOther')"/>
    ...
    <!-- Address -->
    <div class="space-y-4">
      <h3 class="font-semibold text-gray-700">Adres zamieszkania</h3>
      <div class="grid grid-cols-2 gap-4">
        <div>
...
function fieldClass(field) {
  const base =
      'w-full border rounded px-3 py-2 bg-white text-gray-900 focus:outline-none focus:ring-2'
