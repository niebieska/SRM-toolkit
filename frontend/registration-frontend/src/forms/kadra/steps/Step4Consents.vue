  ...
  <div class="bg-white rounded-2xl shadow p-6 space-y-6">
    ...
    <div class="border rounded-lg p-4 space-y-2" :class="errors.consent1 ? 'border-red-400 bg-red-50' : 'border-gray-200'">
      ...
      <p class="text-sm text-gray-600" v-if="isAdult">
        ...
        zgodnie z
        <a :href="rodoUrl" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline font-medium">
    ...
    <!-- Consent 2 —  Regulations (required) -->
    <div class="border rounded-lg p-4 space-y-2" :class="errors.consent3 ? 'border-red-400 bg-red-50' : 'border-gray-200'">
      <h3 class="font-semibold text-gray-700">2. Akceptacja regulaminu *</h3>
      <p class="text-sm text-gray-600" v-if="isAdult">
        Rozumiem charakter rekolekcji oraz wiem, jakich zasad będę {{ musial }} przestrzegać.
        Potwierdzam zapoznanie się z
        <a href="https://rekolekcje.scj.pl/informacje-organizacyjne/" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline font-medium">
          regulaminem
        </a>
      ...
      <p class="text-sm text-gray-600" v-else>
        ...
        i biorę odpowiedzialność za podporządkowanie się jego zasadom przez moje dziecko.
      </p>
      <label class="flex items-center gap-2 cursor-pointer">
    ...
    <!-- Consent 3 — Image consent (optional) -->
    <div class="border border-gray-200 rounded-lg p-4 space-y-2">
      <h3 class="font-semibold text-gray-700">3. Zgoda na wykorzystanie wizerunku <span class="text-gray-400 font-normal text-sm">(Opcjonalna)</span></h3>
      <p class="text-sm text-gray-600" v-if="isAdult">
        Wyrażam zgodę na utrwalanie oraz wykorzystanie mojego wizerunku w formie zdjęć i nagrań wykonanych podczas turnusu SRM,
        a także na jego publikację w materiałach informacyjnych i promocyjnych organizatora (w szczególności na stronach internetowych,
        w mediach społecznościowych oraz materiałach drukowanych). Zgoda jest dobrowolna i może zostać wycofana w dowolnym momencie.
      </p>
      <p class="text-sm text-gray-600" v-else>
        Wyrażam zgodę na utrwalanie oraz wykorzystanie wizerunku mojego dziecka w formie zdjęć i nagrań wykonanych podczas turnusu SRM,
        a także na jego publikację w materiałach informacyjnych i promocyjnych organizatora (w szczególności na stronach internetowych,
        w mediach społecznościowych oraz materiałach drukowanych). Zgoda jest dobrowolna i może zostać wycofana w dowolnym momencie.
      </p>
      <label class="flex items-center gap-2 cursor-pointer">
    ...
    >
      <h3 class="font-semibold text-gray-700">4. Oświadczenie o prawdziwości danych *</h3>
      ...
      <p v-if="errors.truthDeclaration" class="text-red-500 text-xs">{{ errors.truthDeclaration }}</p>
    </div>
...
const rodoUrl = 'https://rekolekcje.scj.pl/polityka-prywatnosci/'  // ← bug fix: was undefined
